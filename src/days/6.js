...
const reduceToString = (arrayOfStrings) =>
  arrayOfStrings.reduce((combined, current) => `${combined}${current}`, '');

const dedupeString = (string) => Array.from(new Set(string.split(''))).join('');

const stringLength = (string) => string.length;

const sum = (a, b) => a + b;

const toOnlyDupes = (arrayOfStrings) =>
  arrayOfStrings.map((string, _, all) =>
    string
      .split('')
      .filter((char) => all.every((entry) => entry.includes(char)))
      .join(''),
  );
