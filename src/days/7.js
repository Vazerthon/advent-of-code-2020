...
const whereListIncludes = (search) => ([, bags]) =>
  Object.keys(bags).includes(search);

const toObject = (acc, [key, value]) => ({ ...acc, [key]: value });

const search = (entries, term, accumulator) => {
  const result = entries.filter(whereListIncludes(term)).reduce(toObject, {});

  const nextSearchTerms = Object.keys(result);
  ...
  return nextSearchTerms
    .map((t) => search(entries, t, combined))
    .reduce((acc, curr) => ({ ...acc, ...curr }), {});
};

const one = (input) => () => {
  const dictOfBags = descriptiveListToDictionary(input);
  const bagsEntries = Object.entries(dictOfBags);

  const result = search(bagsEntries, 'shiny gold', {});
