...
const splitOnBlankLines = (all, curr) =>
  curr === ''
    ? [...all.slice(0, all.length), []]
    : [...all.slice(0, all.length - 1), [...all[all.length - 1], curr]];

const splitOnSpaces = (string) => string.split(' ');

const splitOnColon = (string) => string.split(':');

const flatten = (array) => array.join(' ');

const arrayOfKeyValuePairsToObj = (array) =>
  array.map(splitOnColon).reduce((obj, [k, v]) => ({ ...obj, [k]: v }), {});

const requiredFields = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];

const validatePassportFieldsExist = (obj) =>
  requiredFields
    .map((field) => Object.keys(obj).some((key) => field === key))
...
const fieldValidator = {
  ...
  ecl: (value) =>
    ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].some((v) => value === v),
...
const validatePassportFields = (obj) =>
  Object.entries(obj).reduce(
    (all, [key, value]) => ({
      ...all,
      [key]: tryCatch(() => fieldValidator[key](value)),
    }),
    {},
  );
...
const one = (input) => () =>
  input
    ...
    .map(flatten)
    .map(splitOnSpaces)
    .map(arrayOfKeyValuePairsToObj)
    .map(validatePassportFieldsExist)
